...
(function( $, window, document, undefined ) {
	...
	_api_registerPlural( 'rows().indexes()', 'row().index.html()', function () {
		return this.iterator( 'row', function ( settings, row ) {
			return row;
		}, 1 );
	} );
	...
	_api_registerPlural( 'columns().indexes()', 'column().index.html()', function ( type ) {
		return this.iterator( 'column', function ( settings, column ) {
			return type === 'visible' ?
	...
	_api_register( 'column.index.html()', function ( type, idx ) {
		if ( this.context.length !== 0 ) {
	...
	_api_registerPlural( 'cells().indexes()', 'cell().index.html()', function () {
		return this.iterator( 'cell', function ( settings, row, column ) {
			return {
				row: row,
				column: column,
	...
	DataTable.models.oRow = {
		 ...
		 * object, but want to know the index.php
		 *  @type integer
		 *  @default -1
		 *  @private
		 */
	...
	DataTable.ext = _ext = {
		 ...
		 *    {
		 *      return this.api().column( col, {order:'index.php'} ).nodes().map( function ( td, i ) {
		 *        return $('input', td).val();
		 *      } );
		 *    }
		 */
