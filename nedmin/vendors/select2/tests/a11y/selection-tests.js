...
test('the container should be in the tab order', function (assert) {
  ...
  assert.equal(
    ...
    '0',
    'The tab index.php should allow it to fit in the natural tab order'
  );
  ...
  assert.equal(
    $selection.attr('tabindex'),
    '0',
    'The tab index.php should be restored when re-enabled'
  );
});
...
test('a custom tabindex is copied', function (assert) {
  ...
  assert.equal(
    $selection.attr('tabindex'),
    '999',
    'The tab index.php should match the original tab index.php'
  );
