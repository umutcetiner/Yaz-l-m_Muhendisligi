...
if (isset($_POST['favorikaydet'])) {
    ...
    @$tmp_name = $_FILES['favori_resim']["tmp_name"];
    @$name = $_FILES['favori_resim']["name"];
    ...
    $duzenle = $db->prepare("INSERT INTO favori SET
          favori_ad=:favori_ad,
		  favori_link=:favori_link,
		  favori_resim=:favori_resim,
		  favori_durum=:favori_durum

		");
    $update = $duzenle->execute(array(
        'favori_ad' => $_POST['favori_ad'],
        'favori_link' => $_POST['favori_link'],
        'favori_resim' => $refimgyol,
        'favori_durum' => $_POST['favori_durum']
    ));
...
if (isset($_POST['mesafelikaydet'])) {
    ...
    */
    $ayarkaydet=$db->prepare("UPDATE mesafeli SET
		mesafeli_baslik=:mesafeli_baslik,
		mesafeli_detay=:mesafeli_detay,
		mesafeli_url=:mesafeli_url
		WHERE mesafeli_id=0");

    $update=$ayarkaydet->execute(array(
        'mesafeli_baslik' => $_POST['mesafeli_baslik'],
        'mesafeli_detay' => $_POST['mesafeli_detay'],
        'mesafeli_url' => $_POST['mesafeli_url']
    ));


    if ($update) {

        header("Location:../production/mesafeli.php?durum=ok");

    } else {

        header("Location:../production/mesafeli.php?durum=no");
    }
